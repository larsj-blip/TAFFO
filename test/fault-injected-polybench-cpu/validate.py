...
def ComputeDifference(fixed_point_data, floating_point_data):
  ...
  fixed_point_value_invalid_count = 0
  floating_point_value_invalid_count = 0
  # threshold ???
  ...
  for single_value_fixed_point, single_value_floating_point in zip(fixed_point_data, floating_point_data):
    ...
    # if the fixed point value is infinite, add one to the fixed_point_value_invalid_count value. means one fixed point value was invalid
    if not fixed_point_value.is_finite():
      fixed_point_value_invalid_count += 1
    elif not floating_point_value.is_finite():
      floating_point_value_invalid_count += 1
      fixed_point_value_invalid_count += 1
  ...
  return {'fixed_point_value_invalid_count': fixed_point_value_invalid_count, \
          'floating_point_value_invalid_count': floating_point_value_invalid_count, \
          'average_error_percentage': str(average_error_percentage) + " %" if average_error_percentage != -1 else "no successful iterations",
          'average_error_absolute': average_absolute_error if average_absolute_error != -1 else "no successful iterations"}
